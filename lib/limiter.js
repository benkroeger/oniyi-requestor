...
Limiter.prototype.createBucket = function(callback) {
  ...
  self.redisClient.multi()
    .set(self.keys.remaining, self.limit, 'PX', self.duration, 'NX')
    ...
    .exec(function(err, res) {
      ...
      logDebug('{%s} - created new bucket: remaining {%s}; limit {%s}, reset {%s}', self.id, self.limit, self.limit, new Date(parseInt(expiresAt, null)));
      callback(null, {
...
Limiter.prototype.decreaseRemaining = function(bucket, callback) {
  ...
  self.redisClient.multi()
    ...
    // .expireat(self.keys.remaining, expiresAt)
