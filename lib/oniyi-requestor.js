...
var util = require('util'),
  ...
  cacheEvaluator = require('./cache/evaluator'),
...
function Requestor(options) {
  ...
  self.disableCache = !!options.disableCache;
  if (!self.disableCache) {
    // get the provided cache information per endpoint (host) if any
    self.cacheSettings = _.reduce(options.cache, function(result, conf, endpoint) {
  ...
  self.maxLockTime = (_.isNumber(options.maxLockTime)) ? options.maxLockTime : 5000;
...
Requestor.prototype.addLimit = function(options) {
  var self = this;
  if (!_.isString(options.endpoint)) {
    return false;
  }
  if (!_.isUndefined(self.limiter[options.endpoint])) {
    logWarn('can not overwrite existing limiter for endpoint {%s}', options.endpoint);
    return false;
  }
  return self.limiter[options.endpoint] = new limit(_.merge(_.pick(options, ['limit', 'duration']), {
    id: options.endpoint,
    redisClient: self.redisClient
  }));
}

Requestor.prototype.addCacheSetting = function(options) {
  var self = this;
  if (self.disableCache) {
    logWarn('Can not add cache setting. Caching is disabled for this requestor');
    return false;
  }
  if (!_.isString(options.endpoint)) {
    logWarn('and endpoint must be defined');
    return false;
  }
  if (!_.isUndefined(self.cacheSettings[options.endpoint])) {
    logWarn('can not overwrite existing cache settings for endpoint {%s}', options.endpoint);
    return false;
  }

  return self.cacheSettings[options.endpoint] = _.pick(options, ['storePrivate', 'storeNoStore', 'ignoreNoLastMod', 'requestValidators', 'responseValidators']);
}

Requestor.prototype.throttle = function(opts) {
  var self = this;
  if (self.limiter[opts.parsedUrl.host] && !opts.disableCache) {
    var dummyRequestor;
    self.limiter[opts.parsedUrl.host].getBucket(function(err, bucket) {
      if (err) {
        return opts.callback(err, null);
      }
      if (bucket.remaining < 0) {
        return opts.callback(new Error(util.format('request limit {%d} for {%s} reached, please retry after {%s}', bucket.limit, opts.parsedUrl.host, new Date(parseInt(bucket.reset, null)))));
      }
      dummyRequestor = request(opts.uri, opts, opts.callback);
    });
...
Requestor.prototype.handleRequest = function(options) {
  ...
  var evaluator = new cacheEvaluator(_.merge({}, self.cacheSettings[options.parsedUrl.host], _.pick(options, ['storeNoStore', 'storePrivate', 'ignoreNoLastMod'])));
  ...
  function unlockRequestAndCacheResponse(err, response, body) {
    ...
    if (evaluator && evaluator.isStorable(response)) {
      ...
      if (typeof options.ttl === 'number') {
        ...
        var maxAge = response.headers['cache-control'].match(/s-maxage=([0-9]+)/)[1] || response.headers['cache-control'].match(/max-age=([0-9]+)/)[1];
