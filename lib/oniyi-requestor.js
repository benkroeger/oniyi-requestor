...
function makeCacheableRequest(opts, cacheKeys, redisClient) {
  return request(opts.uri, opts, function requestorCallback(error, response, body) {
    ...
    if (!error && [200].indexOf(response.statusCode) > -1) {
...
Requestor.prototype._request = function(opts) {
  ...
  if (opts.method !== 'GET' || opts.requiresPipe || opts.noCache || !self.redisClient.connected) {
  ...
  if (opts.url) {
    ...
    var requestQuery = encodeURIComponent(querystring.stringify(opts.qs)),
      requestUri = encodeURIComponent(opts.uri),
      cacheKeys = {
        response: util.format('oniyi-requestor:response:%s:%s:%s', opts.authenticatedUser, requestUri, requestQuery),
        raw: util.format('oniyi-requestor:response:body:raw:%s:%s:%s', opts.authenticatedUser, requestUri, requestQuery),
        processed: util.format('oniyi-requestor:response:body:processed:%s:%s:%s', opts.authenticatedUser, requestUri, requestQuery)
      };
...
['delete', 'put', 'get', 'head', 'post'].forEach(function(method) {
...
['defaults', 'forever', 'patch', 'jar', 'cookie'].forEach(function(method) {
  Requestor.prototype[method] = function() {
